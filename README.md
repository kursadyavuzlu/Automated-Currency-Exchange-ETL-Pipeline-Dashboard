...
1. Extract: A Python script safely communicates with the Frankfurter API to fetch daily EUR-based currency exchange rates with built-in timeout and error-handling mechanisms.

2. Transform: The raw JSON response is parsed using Pandas. Data types are validated, unnecessary metadata is stripped, and numerical exchange rates are converted to proper floating-point numbers.

3. Load: Cleaned data is appended into a relational PostgreSQL database schema using psycopg2, preventing duplicate records and maintaining data integrity.

4. Orchestration: Apache Airflow manages the entire workflow as a Directed Acyclic Graph (DAG). It ensures tasks run sequentially, tracks status, and handles data passing between tasks via XComs.

5. Visualization: A Streamlit web application serves as the user-facing analytics portal, querying the live PostgreSQL database to generate dynamic time-series line charts based on user selection.
...
git clone [https://github.com/kursadyavuzlu/Automated-Currency-Exchange-ETL-Pipeline-Dashboard.git](https://github.com/kursadyavuzlu/Automated-Currency-Exchange-ETL-Pipeline-Dashboard.git) cd Automated-Currency-Exchange-ETL-Pipeline-Dashboard
